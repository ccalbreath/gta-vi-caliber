...
| `CasinoGames` | roulette/slots/blackjack | `roulette_spin`, `roulette_payout`, `slot_payout`, `blackjack_settle`, bankroll | **slots + roulette wired live**: `SlotMachine` and `RouletteTable` (both Node3D, group `interactables`) — press interact to bet + spin, paying/charging `PlayerStats`. CI-guarded by `tests/slot_machine_probe.gd` + `tests/roulette_table_probe.gd`. TODO: a blackjack table + a reel/wheel-result HUD |
