...
| `CasinoGames` | roulette/slots/blackjack | `roulette_payout`, `slot_payout`, `blackjack_settle`, bankroll | **slots wired live** via `SlotMachine` (Node3D, group `interactables`): press the interact key to bet + spin, paying/charging `PlayerStats`. CI-guarded by `tests/slot_machine_probe.gd`. TODO: a roulette/blackjack table UI + a reel-result HUD |
