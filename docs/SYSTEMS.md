...
| `HeistJob` | the FACADE that runs a heist end-to-end — owns a `HeistPlan` + `HeistCrew` + `HeistComplication` and resolves the score in one call (crew skill folds into plan risk, complications eat the take, the crew takes its cut) | `plan`, `crew`, `success_chance`, `roll`, `resolve` | wired live via `HeistBoard` (Node3D, group `heist_board`, two Area3D zones Planning+Vault): step into Planning to CASE the joint (each visit completes a prep task, cutting risk + padding the take); once ready, step into the Vault to PULL it — `roll(rng)` then `resolve()` banks the player's cut to `PlayerStats` on a score, or reports a (worse) crime to the `wanted` tracker if caught. The case-then-pull heist loop, verified in `heist_board_probe`. The roll is RNG-injectable for fixed-odds events |
...
| `SmugglingRun` | the run-the-gauntlet TRIP — a bulk cargo run down a ROUTE of legs (open water → patrol box → port), each leg interdicting a slice of what's left, shrunk by your evasion. Distinct from `ContrabandMarket` (carried-parcel arbitrage) and `DealerNetwork` (street network) | `add_leg`, `leg_count`, `cargo_value`, `run`, `to_dict`/`from_dict` | wired live via `SmugglingRoute` (Node3D, group `smuggling_route`, two Area3D zones Pickup+Dropoff): load at Pickup, reach Dropoff to run the gauntlet — `value_delivered` banked to `PlayerStats`, each seizure reported as a crime to the `wanted` tracker. EVASION = `base_evasion` + the player's `PlayerSkills.bonus("driving")` (group `player_skills`), so a trained driver runs cleaner — this is what CONSUMES the gym's driving skill. Verified in `smuggling_route_probe` |
| `Fence` | where stolen VALUABLES become cash — closes the loot→money loop (`StoreRobbery`/`HeistJob`/`LootTable` drop goods; this sells them). Pays a fraction of value, docked for HOT (recently-stolen) goods; let them cool a few days for a better quote. The dump-now-or-sell-clean choice | `add_loot`, `fence_quote`, `cool`, `sell`, `sell_all`, `item_heat`, `inventory_value`, `to_dict`/`from_dict` | wired live as a full loop: owned by `FenceController` (group `fence`, runs the cool-off day clock); a `BurglaryZone` (Area3D, group `burglary`) lifts valuables into the stash on entry (`add_loot`) and draws police heat (reports a crime); a `FenceCounter` (Area3D, group `fence_counter`) sells the whole stash for cash to `PlayerStats` (`sell_all`). Fence it hot for less, or let `FenceController` cool it a few days for a better quote — the dump-now-or-sell-clean choice. The burgle→fence loop, verified in `fence_loop_probe`. Distinct from `ShopModel` (buying), `ChopShop` (vehicles), `ContrabandMarket` (drugs) |
...
| `PlayerSkills` | activity-based proficiency (drive/shoot/stamina...) | `train`, `level`, `tier`, `bonus`, `overall_mastery`, `to_dict` | owned live by `PlayerSkillsController` (group `player_skills`); trained via `TrainingZone` (Area3D, group `training_zone`): step into a gym/range/track and each session raises its `skill_id` (diminishing returns + a rest cooldown so you can't spam-max). Verified in `training_zone_probe`. CONSUMER (partly wired): `SmugglingRoute` reads `bonus("driving")` as evasion (a trained driver runs cleaner) — `smuggling_route_probe`. Remaining: combat reads `bonus("shooting")` for recoil (on the weapon side) |
