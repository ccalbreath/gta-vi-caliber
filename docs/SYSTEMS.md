...
| `RivalRetaliation` | gang VENDETTA: grudge → timed revenge strikes (vandalism → raid → hit squad) | `provoke`, `pacify`, `grudge_of`, `is_seeking_revenge`, `retaliation_kind_for`, `tick`, `serialize` | `provoke(faction, amount)` when the player takes a rival's turf (`GangTerritory.take_over`) / hits them; `tick(delta_days)` decays grudges and returns `{faction_id, kind, severity}` strikes to spawn ambushers/property raids at the player's `PropertyOwnership` holdings; `pacify` for a truce. Faction ids align with `GangTerritory`/`FactionStanding`. Pure |
...
| `PlayerBounty` | a placed cash reward on the PLAYER'S head that draws NPC hunters — distinct from `WantedSystem` (live police heat) and `CrimeNotoriety` (reputation) | `place_bounty`, `total_bounty`, `tier`, `hunter_count`, `threat_level`, `claim`, `pay`, `appease`, `decay` | `place_bounty(faction, amount)` when the player wrongs someone (off `RivalRetaliation`/`FactionStanding`/`CrimeNotoriety`); spawn + scale pursuers from `hunter_count()`/`threat_level()`; resolve via `pay` (wallet, at a fixer), `claim` (on player death, hunter collects), `appease` (private truce), or `decay` (lay low). Persist via `serialize`/`restore`. Pure |
