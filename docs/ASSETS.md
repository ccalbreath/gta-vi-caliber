...
| `game/assets/cars/muscle_gta6.glb` (+ `muscle_gta6_normal.png`, `muscle_gta6_Image_{0,1,3}.jpg`, `muscle_v1_{front,rear,side}.png`) | **EVALUATION — not wired into any scene.** FICTIONAL American-muscle-coupe car body generated via the Higgsfield pipeline (nano-banana-pro reference renders → image-to-3D GLB). The `_Image_*.jpg` and `muscle_v1_*` files are the intermediate reference/turnaround renders kept for re-generation. Generic muscle-car archetype, not modelled on a specific real or in-game vehicle; `gta6` in the name denotes the project's quality bar, not GTA content. | project contributors; generated with Higgsfield (nano-banana-pro + image-to-3D) | original — generic muscle-car prompt | CC BY 4.0 |
| `game/assets/cars/{vice_coupe.glb,vice_coupe_gta6.glb,vice_coupe_hifi.glb}` (+ `vice_coupe*_{normal,ref}.png`, `vice_coupe*_Image_{0,1,3}.jpg`, `vice_coupe_0.png`) | **EVALUATION — not wired into any scene.** FICTIONAL 1980s-style two-door coupe in three fidelity passes (base / `gta6` / `hifi`), generated via the Higgsfield pipeline (nano-banana-pro reference renders → image-to-3D GLB). The `_Image_*.jpg` / `_ref.png` files are the intermediate reference renders kept for re-generation. Generic coupe archetype, not modelled on a specific real or in-game vehicle; `gta6` denotes the quality bar, not GTA content. | project contributors; generated with Higgsfield (nano-banana-pro + image-to-3D) | original — generic coupe prompt | CC BY 4.0 |
