...
| `game/assets/buildings/poc_miami_tower.glb` (+ extracted `poc_miami_tower_*.png`) | Asset-pipeline proof of concept: building massing extruded from the real Miami Tower footprint (OSM way 300232867), dressed with the `facade_tower_01` set, exported from Blender. Scratch/demo asset (see `tests/building_poc_capture.gd`), not placed in any world scene | project contributors; footprint data © OpenStreetMap contributors (ODbL 1.0, https://www.openstreetmap.org/copyright); textures ambientCG (CC0) | original — generated via the docs/ASSET_PIPELINE.md Blender MCP flow | CC BY 4.0 |
